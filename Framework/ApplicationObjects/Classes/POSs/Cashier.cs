...
namespace NSites_V.ApplicationObjects.Classes.POSs
{
    class Cashier
    {
        ...
        public DataTable getAllData(string pDisplayType, string pPrimaryKey, string pSearchString)
        {
            ...
            HttpResponseMessage response = client.GetAsync("api/main/getCashiers?pDisplayType=" + pDisplayType + "&pPrimaryKey=" + pPrimaryKey + "&pSearchString=" + pSearchString + "").Result;
        ...
        public DataTable getCashierDetails(string pUserId)
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(GlobalVariables.BaseAddress);
            HttpResponseMessage response = client.GetAsync("api/main/getCashierDetails?pUserId=" + pUserId + "").Result;
            return response.Content.ReadAsAsync<DataTable>().Result;
        }
            ...
            try
            {
                switch (pOperation)
                {
                    case GlobalVariables.Operation.Add:
                        ...
                        HttpResponseMessage responseAdd = clientAdd.PostAsJsonAsync("api/main/insertCashier/", this).Result;
                    ...
                    case GlobalVariables.Operation.Edit:
                        ...
                        HttpResponseMessage responseEdit = clientEdit.PostAsJsonAsync("api/main/updateCashier/", this).Result;
        ...
        public bool remove(string pId)
        {
            ...
            try
            {
                HttpClient client = new HttpClient();
                client.BaseAddress = new Uri(GlobalVariables.BaseAddress);
                HttpResponseMessage response = client.GetAsync("api/main/removeCashier?pId=" + pId + "&pUserId=" + GlobalVariables.UserId).Result;
