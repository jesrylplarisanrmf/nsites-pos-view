...
namespace NSites_V.ApplicationObjects.Classes.POSs
{
    class ModeOfPayment
    {
        ...
        public DataTable getAllData(string pDisplayType, string pPrimaryKey, string pSearchString)
        {
            ...
            HttpResponseMessage response = client.GetAsync("api/main/getModeOfPayments?pDisplayType=" + pDisplayType + "&pPrimaryKey=" + pPrimaryKey + "&pSearchString=" + pSearchString + "").Result;
        ...
        public DataTable getModeOfPaymentDefault()
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(GlobalVariables.BaseAddress);
            HttpResponseMessage response = client.GetAsync("api/main/getModeOfPaymentDefault").Result;
            return response.Content.ReadAsAsync<DataTable>().Result;
        }
            ...
            try
            {
                switch (pOperation)
                {
                    case GlobalVariables.Operation.Add:
                        ...
                        HttpResponseMessage responseAdd = clientAdd.PostAsJsonAsync("api/main/insertModeOfPayment/", this).Result;
                    ...
                    case GlobalVariables.Operation.Edit:
                        ...
                        HttpResponseMessage responseEdit = clientEdit.PostAsJsonAsync("api/main/updateModeOfPayment/", this).Result;
        ...
        public bool remove(string pId)
        {
            ...
            try
            {
                HttpClient client = new HttpClient();
                client.BaseAddress = new Uri(GlobalVariables.BaseAddress);
                HttpResponseMessage response = client.GetAsync("api/main/removeModeOfPayment?pId=" + pId + "&pUserId=" + GlobalVariables.UserId).Result;
